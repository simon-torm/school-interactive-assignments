...
def main() -> None:
    ...
    assert re.search(r'data-count-for="math">6 завдань<', chooser), "real Math activity count must render"
    ...
    assert catalog.count('class="activity-link"') == 5, "grade 5 filter must render all production Math links"
    assert 'href="activities/math/05-catchx/"' in catalog
    assert 'href="activities/math/05-fractions-workshop/"' in catalog
    ...
    grade6 = rendered_dom("?subject=math&grades=6")
    assert 'href="activities/math/06-percentage-inspector/"' in grade6
