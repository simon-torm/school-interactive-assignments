(function (root, factory) {
  ...
  else root.Grade4LighthouseGame = api;
  ...
  function createState(seed) {
    return Object.freeze({ seed: Number(seed) >>> 0, index: 0, stage: 0, phase: 'ready', hintStep: 0 });
  }
  ...
  function reduce(state, event) {
    ...
    if (event.type === 'HINT') {
      if (!['ready', 'incorrect'].includes(state.phase) || state.hintStep >= 2) return state;
      return Object.freeze({ ...state, hintStep: state.hintStep + 1 });
    }
    ...
    if (event.type === 'CHECK_CORRECT') {
      ...
      return Object.freeze({ ...state, stage, phase: state.index === TOTAL - 1 ? 'finale' : 'correct' });
    }
    if (event.type === 'NEXT') {
      if (state.phase !== 'correct' || state.index >= TOTAL - 1) return state;
      return Object.freeze({ ...state, index: state.index + 1, phase: 'ready', hintStep: 0 });
    }
    return state;
  }
