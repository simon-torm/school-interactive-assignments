(function (root, factory) {
  ...
  function stagePath(value) { return `lighthouse-${normalizeStage(value)}.webp`; }
  ...
  function createController(root, options = {}) {
    ...
    function preloadFollowing(stage) { if (stage < 6) preload(stagePath(stage + 1)).catch(() => {}); }
    ...
    function startTransition() {
      ...
      if (reducedMotion()) { switchImmediately(desiredStage); return; }
      const target = desiredStage;
      const token = ++generation;
      transitionStage = target;
      preload(stagePath(target)).then(() => {
        if (token !== generation) return;
        if (target !== desiredStage) { transitionStage = null; startTransition(); return; }
      ...
      }).catch(() => { if (token === generation) transitionStage = null; });
    }
    function render(projection) {
      const stage = normalizeStage(projection?.stage);
      describe(stage);
      if (stage === 0 || stage < currentStage || reducedMotion()) { switchImmediately(stage); return; }
      desiredStage = stage;
      startTransition();
    }
  ...
  function render(root, projection) {
    ...
    if (!controller) { controller = createController(root); controllers.set(root, controller); }
